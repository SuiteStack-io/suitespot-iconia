    ...
    for (const [roomTypeName, groupReservations] of byRoomType) {
      ...
      for (const res of sorted) {
        ...
        if (bestUnit) {
          ...
          // Step 5: No valid room found
          conflicts.push({
            reservation_id: res.id,
            guest_name: res.guest_names?.[0] || "Unknown",
            check_in: res.check_in_date,
            check_out: res.check_out_date,
            reason: `No available room of type "${roomTypeName}" for the requested dates. All rooms of this type are occupied during part or all of the requested stay.`,
          });
        }
      }
    }
