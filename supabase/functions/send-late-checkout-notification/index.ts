...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const emailPromises = filteredAdmins.map(async (admin: any) => {
      try {
        const emailResponse = await resend.emails.send({
          ...
          html: `
            <!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><meta name="color-scheme" content="light dark"><meta name="supported-color-schemes" content="light dark"><style>@media(prefers-color-scheme:dark){.email-header{background:linear-gradient(135deg,#f59e0b,#d97706)!important}.email-header h1{color:#ffffff!important}}</style></head><body style="margin:0;padding:0;">
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <div class="email-header" style="background: linear-gradient(135deg, #f59e0b, #d97706) !important; padding: 20px; border-radius: 8px 8px 0 0;">
                <h1 style="color: #ffffff !important; margin: 0; font-size: 24px; text-shadow: 0 0 1px rgba(0,0,0,0.5);">⏰ Late Checkout Added</h1>
              </div>
    ...
    console.log(`Late checkout notification emails sent: ${successCount}/${filteredAdmins.length}`);
