...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: userRole } = await supabaseAdmin.from('user_roles').select('role').eq('user_id', user.id).in('role', ['admin', 'super_admin']).maybeSingle();
    if (!userRole) {
    ...
    if (propertyId) {
      ...
      propConfig = {
        ...
        timezone: propertyConfig.timezone || 'UTC',
